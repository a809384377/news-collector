...
- `newsbox.__version__`（运行时 importlib.metadata 取）
- `newsbox --version` CLI 输出
...
def test_dunder_version_matches_pyproject() -> None:
    """`newsbox.__version__` 必须 == pyproject.toml [project].version。

    `__init__.py` 走 `importlib.metadata.version("newsbox")` 读已安装包元数据；
    本测试运行依赖 `uv sync` / `pip install -e .` 把项目装到 venv，metadata 才存在。
    """
    expected = _pyproject_version()
    assert newsbox.__version__ == expected, (
        f"__version__ ({newsbox.__version__}) 与 pyproject.toml ({expected}) 不一致；"
        "若 __version__ 为 '0.0.0+local'，说明 venv 没装本包（跑 uv sync 修复）"
    )


def test_cli_version_flag_matches_dunder() -> None:
    """`newsbox --version` 输出必须含 `__version__` 值。"""
    runner = CliRunner()
    result: Any = runner.invoke(app, ["--version"])
    assert result.exit_code == 0
    assert newsbox.__version__ in result.output
    assert "newsbox" in result.output
