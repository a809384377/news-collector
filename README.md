...
| **协议适配子组件** | RSS / Atom / 网页正文 / X via twikit (cookie-based 直连) / Reddit `.rss` (via RSSHub) / GitHub releases.atom 全打通 |
...
| X (Twitter) 小号 | 抓 X 信源用；留空时 X 类信源不可用，其他信源照常 | 见 [docs/twikit-setup.md](./docs/twikit-setup.md) §1 |
...
**setup 完了但抓 X 信源全失败** — `newsbox doctor` 看 `[Twikit]` panel：cookies 缺失就按 [docs/twikit-setup.md](./docs/twikit-setup.md) §1 步骤生成 `~/.newsbox/twikit_cookies.json`（浏览器 devtools 复制 `auth_token` + `ct0` 两个值）；cookies 已配但仍抓不到，看 [docs/twikit-setup.md](./docs/twikit-setup.md) §3 失败排查（401 / 429 / UserNotFound 区分）。
