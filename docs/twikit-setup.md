...
X (Twitter) 信源的抓取链路。**v1.0.2 起 X 信源从 RSSHub 迁出到 twikit**（s10-x-twikit-migration + s11-twikit-cookieconflict-hotfix），因为 RSSHub 默认 `UserTweets` GraphQL endpoint 对 reply / quote-tweet / self-thread 类条目存在整体截断，机制问题换 token 无解；twikit 通过浏览器 cookie 直连 X GraphQL，三账号实测覆盖率 87–100% 远胜 RSSHub 的 2.9–13%。
...
| | RSSHub UserTweets（v1.0.0 及之前） | twikit（v1.0.2+） |
|---|---|---|
