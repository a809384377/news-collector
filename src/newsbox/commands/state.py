...
- last_fetch_at 显示截断到 19 字符（``YYYY-MM-DD HH:MM:SS``），ISO 8601 微秒和
  时区后缀对人/agent 都没价值（``--json`` 模式下保留原始 ISO 字符串便于程序处理）。
- ``--json`` 输出信息查询类自定义 schema：``{sources: [...], total, failures}``
  （s9 Step 2，详见 commands._json）。
"""
...
def state_cmd(
    ...
    # 统一计算 failure_count，人类视图和 JSON 视图都用到
    failure_count = sum(1 for r in rows if int(r["consecutive_failures"] or 0) > 0)
    ...
    if not rows:
        typer.echo("  (no source state recorded; run newsbox fetch first)")
