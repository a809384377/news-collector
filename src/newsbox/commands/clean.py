"""``newsbox clean`` — 清理 raw.db 中过老的 articles_raw 行。
...
def clean_cmd(
    ...
    if not db_path.exists():
        if json_output:
            emit_err(f"raw.db not found: {db_path}", home=str(home))
        else:
            typer.echo(f"[err] raw.db 未找到: {db_path}", err=True)
        raise typer.Exit(code=1)

    # 解析 --before（注入 _now_utc 让测试锁 ANCHOR，R-6 模式）
    try:
        cutoff = parse_since(before, now=_now_utc())
    except ValueError as exc:
        if json_output:
            emit_err(f"--before parse failed: {exc}", before=before)
        else:
    ...
    if cutoff is None:
        ...
        # 兜底：仍按 exit 2 报错。
        if json_output:
            emit_err("--before is required (e.g. 30d / 7d)", before=before)
        else:
    ...
    try:
        ...
        if not yes:
            ...
            if json_output:
                emit_ok(
                    ...
                    to_delete=cnt_to_delete,
                    to_keep=cnt_keep,
                    total=cnt_total,
                )
                return
            typer.echo("[dry-run] newsbox clean")
            typer.echo("")
            typer.echo(
        ...
        if not json_output:
            typer.echo("[execute] newsbox clean --yes")
            typer.echo("")
            typer.echo(
                f"  cutoff (fetched_at <):  {cutoff_str}  (--before={before})"
            )
