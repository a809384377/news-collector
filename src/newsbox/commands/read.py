...
- 默认 rich Table 输出（参考 commands/state.py 列宽算法）；``--json`` 走 NDJSON
  （流式列表，s9 Step 2 走 ``_json.emit_ndjson`` helper）。
- ``--since`` 默认 "24h"；解析失败 exit 2（``--json`` 模式 emit_err；否则 stderr）。
"""
...
from .. import sdk
from ..utils.throughput import count_articles_raw, gate_read_volume
from ..utils.time import parse_since
from ._helpers import home_option, load_app_config
from ._json import emit_err, emit_ndjson, json_option
...
def read_cmd(
    ...
    yes: bool = typer.Option(
        False,
        "--yes",
        help="跳过调用量阈值软阻断（>10000 条时默认会 confirm；--json 已隐含 --yes）",
    ),
    ...
    # 调用量阈值软阻断（s9 Step 3 / D4）：在 SDK 迭代前 COUNT(*) 预估；
    # 阈值取自 config.thresholds.cli_read_warn（默认 10000）。
    # gate 内部按四态分支决策（详见 utils/throughput.py 模块 docstring）。
    cfg = load_app_config(home)
    predicted = count_articles_raw(
        db_path,
        domain=domain,
        since=since_dt,
    ...
    if as_json:
        emit_ndjson(_article_to_json_dict(art) for art in collected)
