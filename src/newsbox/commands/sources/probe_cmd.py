...
def _probe_to_dict(result: ProbeResult) -> dict:
    """ProbeResult → JSON-friendly dict（保留所有字段）。"""
    return {
        "url": result.url,
...
def sources_probe_cmd(
    ...
    if from_file is not None:
        if not from_file.exists():
            if json_output:
                emit_err(
                    f"--from-file path does not exist: {from_file}",
                    path=str(from_file),
                )
                raise typer.Exit(code=2)
            raise typer.BadParameter(f"--from-file 路径不存在: {from_file}")
        urls = _read_url_file(from_file)
        if not urls:
            if json_output:
                emit_err(
                    f"{from_file} 没有有效 URL（空文件 / 全注释行）",
                    path=str(from_file),
                )
            else:
                typer.echo(
        ...
        if json_output:
            ...
            emit(
                {
                    "results": [_probe_to_dict(r) for r in results],
                    "total": total,
                    "reachable": ok,
