"""``newsbox sources list`` / ``show`` 子命令实装。
...
- enabled 字段缺省视为 true（与 ``newsbox.sources`` 已有约定一致）
"""
...
def sources_list_cmd(
    ...
    if json_output:
        ...
        for kind, tier, src_id, enabled, url in rows:
            ...
            if enabled:
                by_kind[kind]["enabled"] += 1
            by_kind[kind]["sources"].append(
                {
                    "id": src_id,
                    "tier": tier,
                    "enabled": enabled,
...
def sources_show_cmd(
    ...
    if json_output:
        ...
        # plain 是 dict（item 一定是 mapping）
        if not isinstance(plain, dict):
            plain = {}
        # 缺省 enabled 视为 true（与人类视图一致）
        payload = {
            "id": plain.get("id", source_id),
            "type": kind,
            "index": idx,
            "enabled": bool(plain.get("enabled", True)),
            **{k: v for k, v in plain.items() if k not in ("id", "enabled")},
        }
        emit(payload)
        return
