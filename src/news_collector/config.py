...
2. 用户 ~/.news-collector/config.yaml（deep merge 覆盖）
3. ~/.news-collector/.env（python-dotenv → Secrets）
...
DEFAULT_HOME = Path.home() / ".news-collector"
...
def _load_default_yaml() -> dict[str, Any]:
    ...
    res = resource_files("news_collector._defaults") / "config.default.yaml"
