"""``news-collector teardown`` — 停容器，数据保留。

设计取舍：
- 只关 RSSHub + Redis 两个 compose 服务，**不删除任何用户数据**
  （docker-compose.yml 用绑定挂载到 ``~/.news-collector/rsshub/``，
  ``docker compose down`` 不会动绑定挂载的宿主目录）。
- 不提供 ``--purge`` 选项：避免 agent 误操作删 raw.db / 历史数据；
  用户若要彻底清理，可手工 ``rm -rf ~/.news-collector``，应明确告知后果。
...
def teardown_cmd(home: Path = home_option()) -> None:
    """停止容器（数据保留在 ~/.news-collector/）。"""
