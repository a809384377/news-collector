"""``news-collector restart`` — 重启 RSSHub 容器。
...
- 重启后短暂轮询（最多 5 秒）等待容器恢复 Up；时间窗内未恢复也只发 [warn]
  ...
  并不等于硬失败，建议用户跑 ``news-collector status`` 进一步看。
...
def restart_cmd(home: Path = home_option()) -> None:
    ...
    if final_state == "Up":
        ...
        typer.echo(
            ...
            f"{_WAIT_TOTAL_SECONDS}s; check 'news-collector status'"
        )
