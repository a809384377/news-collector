"""``news-collector sources test`` 子命令实装。
...
❗本命令**不写** ``sources.yaml``，也**不写** SQLite。要落库走 ``news-collector fetch``。
...
def sources_test_cmd(
    ...
    用法::
        ...
        news-collector sources test x_dotey
        news-collector sources test anthropic_news --limit 5
    """
    ...
    try:
        ...
        typer.echo(
            ...
            "      运行 `news-collector sources seed` 初始化",
            err=True,
        )
