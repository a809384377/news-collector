...
- 默认 db_path = ~/.news-collector/raw.db；调用方可覆盖（测试 / 多 db 场景）。
...
_DEFAULT_DB_PATH = Path.home() / ".news-collector" / "raw.db"
...
def read_raw(
    ...
    参数:
        ...
        db_path: 覆盖默认 ~/.news-collector/raw.db；测试 / 多 db 场景使用。
    ...
    if not db.exists():
        raise FileNotFoundError(
            f"raw.db 未找到：{db}. 请先运行 news-collector fetch 落库，"
