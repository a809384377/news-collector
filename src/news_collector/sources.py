...
- ``seed_sources``  把 sources.seed.yaml 拷到 ~/.news-collector/sources.yaml
- ``list_sources``  按 2 类（rss/web）统计条目数与启用数
- ``iter_sources``  返回完整条目列表（带 source_type 字段，已 enabled 过滤）— fetch 编排消费

模块独立：不依赖 news_collector.config / news_collector.cli，便于薄封装与测试。
"""
...
# 种子清单作为 package data 内置（``news_collector.data``）；pipx / uv tool install
...
DEFAULT_SEED_PATH = Path(
    str(files("news_collector.data").joinpath("sources.seed.yaml"))
)
